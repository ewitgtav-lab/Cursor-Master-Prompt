...
def extract_text_from_pdf(pdf_bytes: bytes) -> str:
    ...
    chunks: list[str] = []
    with pdfplumber.open(io=pdf_bytes) as pdf:  # type: ignore[arg-type]
...
def extract_document(file_name: str, mime_type: str, data: bytes) -> ExtractedDocument:
    if mime_type == "application/pdf" or file_name.lower().endswith(".pdf"):
        ...
        if not text.strip():
            ...
            ocr_chunks: list[str] = []
            try:
                with pdfplumber.open(io=data) as pdf:  # type: ignore[arg-type]
                    for page in pdf.pages:
                        try:
...
def run_gemini(system_prompt: str, user_prompt: str, *, timeout_hint_seconds: int = 40) -> str:
    ...
    if not api_key:
        raise RuntimeError(
            "Missing Gemini API key. Set GEMINI_API_KEY in Streamlit Secrets or as an environment variable."
        )

    genai.configure(api_key=api_key)

    # Prefer a broadly available model name. Users can update later.
    ...
    try:
        resp = model.generate_content(
            ...
            request_options={"timeout": timeout_hint_seconds},
        )
    except TypeError:
        # Older client versions may not support request_options; retry without it.
        resp = model.generate_content(
            user_prompt,
            generation_config={
                "temperature": 0.3,
        ...
        raise RuntimeError("Gemini request failed (timeout or API error). Please try again.") from e
...
def _render_sidebar() -> None:
    with st.sidebar:
        ...
        st.page_link("app.py", label="The Clarity Bridge", icon="🔎")
        st.markdown(
...
""",
    ...
    if not extracted.text.strip():
        st.error(
            "I couldn't extract readable text from that file. If it's scanned, enable OCR by installing Tesseract (pytesseract) in your environment."
        )
        st.stop()
