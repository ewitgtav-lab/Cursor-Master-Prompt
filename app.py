...
""",
    ...
    if not extracted.text.strip():
        ...
        st.caption(
            f"File: {uploaded.name} • Type: {mime_type or 'unknown'} • OCR available: {'yes' if ocr_ready else 'no'}"
        )
        st.markdown(
            "- If this is a **scanned** document (image-only PDF / photo), you need OCR.\n"
            "- Install the **Tesseract** app on your system and ensure the `tesseract` command is available.\n"
            "- Then keep `pytesseract` installed (already in `requirements.txt`)."
        )
        st.stop()
